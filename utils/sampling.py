...
def generalized_steps_overlapping(x, x_cond, seq, model, b, eta=0., corners=None, p_size=None, manual_batching=False,
                                  device=None):
    with torch.no_grad():
        n = x.size(0)
        # n = corners.size(0)
        # print(len(corners)) # 16
        seq_next = [-1] + list(seq[:-1])
        x0_preds = []
        xs = [x]
